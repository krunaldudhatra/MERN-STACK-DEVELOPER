...
class ExpenseRepository {
  ...
  async updateTagInExpense(id, oldTag, newTag) {
    ...
    const result = await db
        .collection(this.collectionName)
        .updateOne(
            { _id: new ObjectId(id), tags: oldTag }, // Find the expense with the old tag
            { $set: { "tags.$": newTag } } // Update the matched tag
        );
  ...
  async deleteTagFromExpense(id, tag) {
    ...
    const result = await db
        .collection(this.collectionName)
        .updateOne(
            { _id: new ObjectId(id) }, // Find the expense by its ID
            { $pull: { tags: tag } } // Remove the specified tag
        );
